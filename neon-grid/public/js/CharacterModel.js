...
export function buildCharacterModel(classColor, playerClass, username, level) {
  ...
  // ── Materials ──────────────────────────────────────────────────────────────
  // Lambert catches scene light AND has emissive so it's visible in dark scenes
  const suit  = () => new THREE.MeshLambertMaterial({ color: 0x1a1a35, emissive: new THREE.Color(ec).multiplyScalar(0.18) });
  const armor = () => new THREE.MeshLambertMaterial({ color: 0x22224a, emissive: new THREE.Color(ec).multiplyScalar(0.55) });
  const glow  = () => new THREE.MeshBasicMaterial({ color: ec });
  const visor = () => new THREE.MeshBasicMaterial({ color: ec });
  const dark  = () => new THREE.MeshLambertMaterial({ color: 0x111122, emissive: new THREE.Color(0x111122) });
  const boot  = () => new THREE.MeshLambertMaterial({ color: 0x0e0e1e, emissive: new THREE.Color(0x080810) });
  ...
  // ── Per-character light so the model is always visible ────────────────────
  const charLight = new THREE.PointLight(ec, 1.8, 4.5);
  charLight.position.set(0, 1.3, 0);
  group.add(charLight);
  ...
  box(0.44, 0.38, 0.08, armor(), bodyGroup, 0, 1.08, -0.19);   // chest plate (-Z = front)
  box(0.50, 0.24, 0.28, suit(),  bodyGroup, 0, 0.82, 0);
  ...
  // Neon strips
  box(0.38, 0.032, 0.032, glow(), bodyGroup, 0, 1.06, -0.24);  // chest strip
  box(0.032, 0.44, 0.032, glow(), bodyGroup, 0, 1.02, 0.18);   // spine strip

  // ── Head ──────────────────────────────────────────────────────────────────
  const headGrp = grp(bodyGroup, 0, 1.56, 0);
  box(0.42, 0.46, 0.42, suit(),  headGrp);
  box(0.30, 0.11, 0.38, armor(), headGrp, 0,  0.30, 0);        // top ridge
  box(0.34, 0.20, 0.060, visor(), headGrp, 0,  0.04, -0.235);  // VISOR on -Z face
  box(0.055, 0.32, 0.22, armor(), headGrp, -0.25, 0, -0.10);   // cheek L
  box(0.055, 0.32, 0.22, armor(), headGrp,  0.25, 0, -0.10);   // cheek R
  box(0.30,  0.08, 0.09, armor(), headGrp, 0, -0.24, -0.18);   // chin

  // ── LEFT ARM ──────────────────────────────────────────────────────────────
  const lShoulder = grp(bodyGroup, -0.42, 1.24, 0);
  box(0.18, 0.38, 0.18, suit(),  lShoulder, 0, -0.19, 0);
  box(0.032, 0.32, 0.032, glow(), lShoulder, 0, -0.12, -0.10); // arm strip

  const lElbow = grp(lShoulder, 0, -0.38, 0);
  box(0.16, 0.30, 0.16, suit(), lElbow, 0, -0.15, 0);
  ...
  box(0.22, 0.36, 0.22, suit(),  lHip, 0, -0.18, 0);
  box(0.22, 0.12, 0.07, armor(), lHip, 0, -0.34, -0.12);       // knee cap
  box(0.032, 0.30, 0.032, glow(), lHip, 0, -0.14, -0.12);      // thigh strip

  const lKnee = grp(lHip, 0, -0.36, 0);
  box(0.20, 0.32, 0.20, suit(),  lKnee, 0, -0.16, 0);
  ...
  box(0.032, 0.30, 0.032, glow(), rHip, 0, -0.14, -0.12);
  ...
  group.neon_animate = (dt, velocity, isShooting, isSprinting) => {
    ...
    if (anim.shootTimer > 0) {
      ...
      // RUN — weapon raised, mild arm swing
    ...
    // Pulse character light with sprint/shoot state
    charLight.intensity = sprinting ? 2.4 : isShooting ? 3.5 : 1.8;
  };
